#include <stdio.h>
#include <windows.h>
#include <unistd.h>  // Để sử dụng usleep

void openUrlInBrowser(const char* url) {
    // Gọi ShellExecute để mở trình duyệt với URL
    ShellExecute(0, "open", url, 0, 0, SW_SHOWNORMAL);
}

// Hàm thiết lập màu cho console
...
void loadingBar(int total)
{
    for (int i = 0; i <= total; i++)
    {
        ...
        fflush(stdout);
        usleep(300000);  // Tạm dừng 0.1 giây (100,000 micro giây)
    }
}
...
void toolFb(){
    int fb;
    const char* url = "https://www.youtube.com/watch?v=Xhpv8dLynfA";
    // Sau khi thanh loading hoàn tất, yêu cầu nhập link Facebook
    SET_COLOR(12);
    printf("\nNhap link Facebook vao day: ");
    scanf("%d", &fb);  // Đây là một cách đơn giản, có thể cải thiện bằng cách nhập chuỗi nếu cần.

    // Hiển thị một thanh loading trước khi nhập link Facebook
    SET_COLOR(10);
    printf("\nTool dang khoi dong...\n");
    loadingBar(50);  // Thanh loading sẽ có 50 bước


    // Hiển thị thông báo sau khi nhập xong
    SET_COLOR(14);
    openUrlInBrowser(url);
    printf("Chuong trinh hoat dong thanh cong ...\n");
}
...
void loadingBar2(int total) {
    for (int i = 0; i <= total; i++) {
        ...
        fflush(stdout);
        usleep(100000);  // Tạm dừng 0.1 giây (100,000 micro giây)
    }
...
int main() {
    ...
    while (again) {
        ...
        switch (menu) {
        case '1':
            loadingBar(50);  // Thanh loading cho 'cho di hoc'
            choDiHoc();
            break;
        case '2':
            loadingBar2(50);  // Thanh loading cho 'cho di an'
            break;
        case '3':
            loadingBar3(50);  // Thanh loading cho 'cho di choi'
            break;
        case '4':
            loadingBar4(50);  // Thanh loading cho toolFb
            break;
        default:
            SET_COLOR(12);
